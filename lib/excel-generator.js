...
// ============================================================
// STANDARD COLUMNS for accepted email sheets
// ============================================================
...
function buildSummarySheet(stats, fileName, durationMs) {
  ...
  const rows = [
    ...
    ['', '', '', ''],
    ['TAGS (overlap with primary categories)', '', '', ''],
    ['Trade-Based (all)', stats.tradeBased || 0, pct(stats.tradeBased || 0), 'Construction/electrical/plumbing keywords'],
    ['Role-Based (all)', stats.roleBased || 0, pct(stats.roleBased || 0), 'info@, admin@, etc.'],
    ['Catch-All Signal (all)', stats.catchAll || 0, pct(stats.catchAll || 0), 'Heuristic based, DNS-only detection'],
    ['Fixed (all)', stats.fixed || 0, pct(stats.fixed || 0), 'TLD or format corrections applied'],
    ['', '', '', ''],
    ['CROSS-BREAKDOWN', '', '', ''],
    ['Google Workspace + Trade-Based', stats.googleWorkspaceTradeBased || 0, pct(stats.googleWorkspaceTradeBased || 0), 'Trade keywords on Google MX'],
    ['Google Workspace + Role-Based', stats.googleWorkspaceRoleBased || 0, pct(stats.googleWorkspaceRoleBased || 0), 'Role addresses on Google MX'],
    ...
    ['Non-Google + Trade-Based', stats.nonGoogleTradeBased || 0, pct(stats.nonGoogleTradeBased || 0), 'Trade keywords on other providers'],
    ['Non-Google + Role-Based', stats.nonGoogleRoleBased || 0, pct(stats.nonGoogleRoleBased || 0), 'Role addresses on other providers'],
    ['Non-Google + Catch-All', stats.nonGoogleCatchAll || 0, pct(stats.nonGoogleCatchAll || 0), 'Other providers with catch-all signals'],
    ['', '', '', ''],
    ['NOTES', '', '', ''],
    ['Category vs Tags', 'Explanation', '', 'Each email has ONE primary category (Google/Non-Google/Removed) plus optional tags (Trade, Role, Catch-All)'],
    ['Excel Sheet Structure', 'Reference', '', 'Google & Non-Google sheets show ALL emails in that category. Sub-sheets filter by tag.'],
  ...
  ws['!cols'] = [{ wch: 40 }, { wch: 18 }, { wch: 14 }, { wch: 55 }];
  styleSheet(ws);
  freezeTopRow(ws);
  return ws;
}
...
/**
 * Build a filtered sheet from records matching a predicate
 */
function buildFilteredSheet(records, predicate, includeStandardHeader = true) {
...
/**
 * Build Removed sheet (different columns)
 */
...
/**
 * Build Full Log sheet - complete audit trail
 */
...
function generateExcel(records, stats, outputPath, fileName, durationMs) {
  ...
  // ============================================================
  // Sheet 2: All Google Workspace emails (everything with Google MX)
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r => r.categorization.category === CATEGORIES.GOOGLE_WORKSPACE),
    'Google Workspace'
  );

  // Sheet 3: Google Workspace WITHOUT any tags (the pure list)
  XLSX.utils.book_append_sheet(wb,
  ...
  // Sheet 4: Google Workspace + Trade-Based
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r =>
      r.categorization.category === CATEGORIES.GOOGLE_WORKSPACE &&
      r.categorization.tags.isTradeBased
  ...
  // Sheet 5: Google Workspace + Role-Based
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r =>
      r.categorization.category === CATEGORIES.GOOGLE_WORKSPACE &&
      r.categorization.tags.isRoleBased
  ...
  // Sheet 6: Google Workspace + Catch-All
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r =>
      r.categorization.category === CATEGORIES.GOOGLE_WORKSPACE &&
      r.categorization.tags.isCatchAll
  ...
  // Sheet 9: Non-Google + Trade-Based
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r =>
      r.categorization.category === CATEGORIES.NON_GOOGLE &&
      r.categorization.tags.isTradeBased
  ...
  // Sheet 10: Non-Google + Role-Based
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r =>
      r.categorization.category === CATEGORIES.NON_GOOGLE &&
      r.categorization.tags.isRoleBased
  ...
  // ============================================================
  // TAG-BASED SHEETS (all matching emails regardless of primary category)
  // ============================================================
  // Sheet 12: All Trade-Based (across categories)
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r => r.categorization.tags && r.categorization.tags.isTradeBased),
    'All Trade-Based'
  );

  // Sheet 13: All Role-Based (across categories)
  XLSX.utils.book_append_sheet(wb,
    buildFilteredSheet(records, r => r.categorization.tags && r.categorization.tags.isRoleBased),
    'All Role-Based'
  );
