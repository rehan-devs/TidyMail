...
/**
 * Update the status.json file for a job (safe against write failures)
 */
...
async function runPipeline(jobId, inputFilePath, jobDir) {
  ...
  try {
    ...
    // ============================================================
    // STAGE 2: CLEANING (with per-email error isolation)
    // ============================================================
    updateStatus(jobDir, {
      status: 'cleaning',
    ...
    for (const emailObj of rawEmails) {
      try {
        ...
        cleanErrors++;
        console.error(`[PIPELINE] Cleaning error on email ${cleanProgress + 1}:`, err.message);
        removedInCleaning.push({
          original: String(emailObj.raw || emailObj),
          cleaned: '',
          localPart: '',
          domain: '',
          cleanStatus: 'removed',
    ...
    console.log(`[PIPELINE] Cleaning complete: ${cleanedRecords.length} survived, ${removedInCleaning.length} removed, ${cleanErrors} errors`);
    ...
    // ============================================================
    // STAGE 3: TLD FIXING (with per-email error isolation)
    // ============================================================
    updateStatus(jobDir, {
      status: 'fixing',
      currentStep: 'Correcting TLD typos...',
      progress: 0,
    ...
    for (const record of cleanedRecords) {
      try {
        ...
        fixErrors++;
        console.error(`[PIPELINE] TLD fix error on email ${fixProgress + 1}:`, err.message);
      ...
      if (fixProgress - lastFixUpdate >= 1000 || fixProgress === cleanedRecords.length) {
        ...
        updateStatus(jobDir, {
          ...
          currentStep: `Fixing TLDs... ${fixProgress} of ${cleanedRecords.length} (${tldFixCount} fixed)`,
        });
      }
    }

    console.log(`[PIPELINE] TLD fixing complete: ${tldFixCount} fixed, ${fixErrors} errors`);

    updateStatus(jobDir, {
      progress: cleanedRecords.length,
      currentStep: `TLD fixing complete. Corrected ${tldFixCount} addresses.`,
    });

    // ============================================================
    // STAGE 4: VALIDATION (with per-email error isolation)
    // ============================================================
    updateStatus(jobDir, {
      status: 'validating',
      currentStep: 'Validating email format...',
      progress: 0,
      total: cleanedRecords.length,
    });
    ...
    for (const record of cleanedRecords) {
      try {
        ...
        validErrors++;
        console.error(`[PIPELINE] Validation error on email ${validProgress + 1}:`, err.message);
    ...
    console.log(`[PIPELINE] Validation complete: ${validRecords.length} valid, ${invalidRecords.length} invalid, ${validErrors} errors`);

    updateStatus(jobDir, {
      progress: cleanedRecords.length,
      currentStep: `Format validation complete. ${validRecords.length} valid emails.`,
    });
    ...
    console.log(`[PIPELINE] Dedup complete: ${dupCount} duplicates removed, ${dedupedRecords.length} unique`);

    updateStatus(jobDir, {
      currentStep: `Removed ${dupCount} duplicates. ${dedupedRecords.length} unique emails.`,
    ...
    updateStatus(jobDir, {
      status: 'dns',
      currentStep: 'Checking domain existence...',
      progress: 0,
      total: dedupedRecords.length,
    });
    ...
    updateStatus(jobDir, {
      status: 'mx',
      currentStep: `Checking ${dnsTotal} unique domains...`,
      progress: 0,
      total: dnsTotal,
    });
    ...
    let dnsResults;
    try {
      dnsResults = await checkEmailsBatch(
        emailStrings,
        (completed, total, currentDomain, domainResult) => {
          try {
            ...
            console.error('[PIPELINE] Progress callback error:', cbErr.message);
          }
        }
      );
    } catch (dnsErr) {
      console.error('[PIPELINE] DNS batch failed:', dnsErr.message);
      dnsResults = emailStrings.map((email, i) => ({
        email,
        domain: dedupedRecords[i].domain,
        domainResult: {
          domain: dedupedRecords[i].domain,
          exists: false,
          hasMX: false,
          isGoogle: false,
          provider: 'Unknown',
          catchAll: { likelyCatchAll: false, confidence: 'low', reason: 'DNS batch error' },
    ...
    updateStatus(jobDir, {
      currentStep: `DNS verification complete for ${dnsTotal} domains.`,
      progress: dnsTotal,
    });
    ...
    let categorizedResults, stats;
    try {
      const catResult = categorizeAll(allRecordsForCategorization);
      categorizedResults = catResult.results;
      stats = catResult.stats;
    } catch (catErr) {
      console.error('[PIPELINE] Categorization error:', catErr);
      throw catErr;
    }
    ...
    try {
      ...
      console.error('[PIPELINE] Excel generation error:', excelErr);
      throw excelErr;
    }
    ...
    console.log(`[PIPELINE] Trade-Based: ${stats.tradeBased}`);
    console.log(`[PIPELINE] Role-Based: ${stats.roleBased}`);
